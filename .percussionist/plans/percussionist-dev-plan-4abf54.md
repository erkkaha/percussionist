...
**Revision 2 (retry):** PLAN review directed the A3 resolution: *"A3 fix docs.
Tool should work. Create run should work but in a way that is intended not as
falsely documented."* Revision 2 removes the previous "decide with maintainers"
open question: A3 is now **decided** (restore the documented `pending → running`
contract in the `create_run` handler and fix every doc site that describes it),
and BUILD-3 is rewritten with the concrete, ordered implementation steps plus
the missing success-path tests (C7). All other findings were re-verified against
the current branch and are unchanged; the A2 wording was tightened to match what
its test actually asserts.
...
| A3 | **High (verified) — resolved in rev 2, see decision below** | `create_run` MCP tool documents "The task must be in the 'pending' phase" but `isValidTransition('pending', 'running')` is false (`TRANSITION_TABLE.pending = ['scheduled']`); the only phases with a `→ running` edge are `initializing` and `waiting-for-input`. The tool is unusable for exactly the phase it advertises. This is a **refactor regression**: before the decision-engine refactor (commit `f435753~1`) the handler enforced `if (currentPhase !== 'pending') throw` and created the run directly — matching its description; commit `f435753` switched the gate to the transition table without touching the description, silently breaking the documented primary use ("Direct run creation without waiting for reconcile cycle" — i.e. schedule a backlog task now). Its success path has zero test coverage (only the capability-failure path is tested). **Decision:** restore the documented contract — allow `pending → running` in the `create_run` handler **only** (not the global `TRANSITION_TABLE`), keep table validation for all other phases, fix the description + AGENTS.md + reference docs, and add success-path tests. Rationale in Risks below; fallback (description-only fix) noted there too. | `packages/manager-controller/src/agent/tools.ts:232-234`, `:1353-1360`; `packages/api/src/index.ts:891-915`; pre-refactor behavior at `f435753~1` |
...
3. **BUILD-3 — Fix `create_run` doc/validation mismatch (A3, C7).** **Decided approach** (per PLAN-review direction "fix docs; tool should work"): restore the documented `pending → running` contract in the `create_run` handler and make every doc site truthful. Do **not** modify the global `TRANSITION_TABLE`.
   1. `packages/manager-controller/src/agent/tools.ts` `create_run` handler (lines 1353-1360): change the gate from `!isValidTransition(currentPhase, 'running')` to
      `const canStartRun = currentPhase === 'pending' || isValidTransition(currentPhase, 'running');`
      with a comment stating `pending` is the tool's documented admin contract (pre-decision-engine behavior, restored) and all other phases must permit `→ running` per `TRANSITION_TABLE`. Keep the existing error message for rejected phases (it already names allowed transitions and points at `force_retry`).
   2. Update the tool description (tools.ts:233-234) to state the contract truthfully, e.g.: "Create a new Run for a task directly (admin shortcut; bypasses the reconcile cycle). The task must be 'pending' (backlog — the run is created immediately) or in a phase that permits transition to 'running' per the transition table ('initializing', 'waiting-for-input'). For 'failed'/'rework-requested' tasks use `force_retry`, which resets the worker and increments retryCount."
   3. Sync the secondary doc sites: AGENTS.md `create_run` section (lines ~577-584 — drop the "use `force_retry` first" framing for `pending`, add the pending exception) and `docs/reference/mcp-tools.md` (line 15 row + line 244 mention if needed). `docs/task-lifetime.md:8` is generic and needs no change.
   4. Add success-path tests (fixes C7) in a new `packages/manager-controller/src/agent/__tests__/tools-create-run.test.ts` (mock `@percussionist/kube` as in `tools-capability-gating.test.ts`, plus `mock.module('../worker-builder.js')` for `buildWorkerRun`/`resolveAgentModel`, or spy on them; call via the existing `__test.handleMcp` harness, `tools.ts:2887-2889`):
      - `pending` task → run created with `workerRunName(project, task, 0)`, task phase → `running`, `worker.runName`/`status: 'Running'`/feature-branch metadata patched (assert the exact patch via `patchTaskStatus` mock);
      - `initializing` and `waiting-for-input` still succeed (valid transitions);
      - `scheduled`, `failed`, `done` → rejected with the "cannot create run / use force_retry" error, no patch, no run creation;
      - `AlreadyExists` with a `Failed`/`Cancelled` run → delete run + `cleanupRunWorktree` + recreate, returns `phase: 'Created'`;
      - `AlreadyExists` with an active run → error, task left untouched;
      - auth-validation failure (`validateModelAuth`) → throws, task left in `pending` (run not created, phase not patched);
      - agent override with a different model → `workerRun.spec.model` re-resolved via `resolveAgentModel` for the overriding agent.
   *Files:* `packages/manager-controller/src/agent/tools.ts`, `AGENTS.md`, `docs/reference/mcp-tools.md`, new `tools-create-run.test.ts`.
...
- **A3 decision (rev 2) — restored `pending → running`, docs fixed.** Rationale: the tool has documented the `pending` contract since inception and its stated purpose (AGENTS.md) is "Direct run creation without waiting for reconcile cycle"; the decision-engine refactor broke the behavior while leaving the docs. A docs-only fix (rewrite the description to say `initializing`) would leave the tool unable to schedule backlog tasks — the one use the docs and the pre-refactor code both intended — so the handler gains a `pending` exception while the global `TRANSITION_TABLE` stays untouched (the reconciler pipeline `pending → scheduled → initializing → running` and CLI `board task move` semantics are unchanged). **Residual race:** if `create_run` runs concurrently with the reconciler deciding `pending → scheduled`, whichever patches first wins; the effect executor re-validates the source phase before applying (`effects.ts:81-89`), and if `create_run`'s run creation fails afterwards it resets the task to `pending` (`tools.ts:1429-1431`) — both directions are self-healing, same as `force_retry`/`set_task_state`. BUILD-3 must assert this in the tests (concurrent-phase-change → no double run). If a maintainer still prefers the tool to be `initializing`-only, the fallback is a description-only fix — but that contradicts the documented purpose and is not what the review directed.
