...
**Revision 3 (retry 3/3):** PLAN review rejected the rev-2 A3 resolution: *"A3
rejected, all runs should go through reconciliation. no poking holes to the
allowed transitions."* Revision 3 removes the `pending → running` exception
entirely. A3 is now **decided** as: `create_run` stops creating Run CRs and
stops special-casing any phase — it becomes a *schedule-now* tool that advances
a task one legal step (`pending`/`rework-requested` → `scheduled`, existing
`TRANSITION_TABLE` edges only) and the reconciler creates the run on its next
cycle through the `ScheduleRun` effect. No transition-table hole, no direct run
creation. BUILD-3 is rewritten with the concrete implementation steps and the
missing success-path tests (C7). All other findings were re-verified against the
current branch and are unchanged from rev 2.
...
| A2 | **High (verified)** | Manager `buildPayloads` never populates `toolCallsPayload` — the part loop pushes only to `fileOpsPayload`; `toolCallsPayload` is declared, never written, and returned as `[]`. Every manager chat-session's tool metrics are silently lost in the web Stats view; the manager's test suite never asserts a populated `toolCallsPayload` (the only toolCalls assertion is `toEqual([])` for empty input, which cannot catch the bug). The dispatcher copy (which the comment at line 44 claims to mirror) does correlate tool-use/tool-result. | `packages/manager-controller/src/agent/stats-reporter.ts:55-149` |
| A3 | **High (verified) — resolved in rev 3, see decision below** | `create_run` MCP tool documents "The task must be in the 'pending' phase" but `isValidTransition('pending', 'running')` is false (`TRANSITION_TABLE.pending = ['scheduled']`); the only phases with a `→ running` edge are `initializing` and `waiting-for-input`. The tool is unusable for exactly the phase it advertises. This is a **refactor regression**: before the decision-engine refactor (commit `f435753~1`) the handler enforced `if (currentPhase !== 'pending') throw` and created the run directly — matching its description; commit `f435753` switched the gate to the transition table without touching the description, silently breaking the documented primary use. Its success path has zero test coverage (only the capability-failure path is tested). **Decision (rev 3):** make the tool truthful *and* compliant — `create_run` no longer creates Run CRs at all and no phase is special-cased. It transitions `pending`/`rework-requested` → `scheduled` (existing table edges) and the reconciler creates the run via the `ScheduleRun` effect. The global `TRANSITION_TABLE` is untouched and no new edges are introduced. Rationale in Risks below. | `packages/manager-controller/src/agent/tools.ts:232-234`, `:1330-1443`; `packages/api/src/index.ts:891-915`; pre-refactor behavior at `f435753~1` |
...
- **Out of scope:** changing `force_retry`'s direct-run-creation path (see Risks — flagged for a future maintainer decision, not part of this review's fix list).
...
2. **BUILD-2 — Populate manager `toolCallsPayload` (A2, B5).** Port the dispatcher's tool-use/tool-result correlation (success/error/duration) into the manager `buildPayloads`; extend the manager test suite to assert populated tool-call rows with the same shape as the dispatcher's (the current suite only checks `toolCallsPayload` is `[]` for empty input). *Files:* `packages/manager-controller/src/agent/stats-reporter.ts` + its test.
3. **BUILD-3 — Rework `create_run` to schedule through reconciliation (A3, C7).** **Decided approach** (per PLAN-review retry direction: *"all runs should go through reconciliation; no poking holes to the allowed transitions"*): `create_run` stops creating Run CRs and stops special-casing any phase. It advances a task one legal step into the standard pipeline via existing `TRANSITION_TABLE` edges and the reconciler creates the run on its next cycle (Task informer enqueues the project on the phase patch; `decideScheduled` emits a `ScheduleRun` effect; `effects.ts` calls `buildWorkerRun` + `createRun`). The global `TRANSITION_TABLE` is **not** modified.
   1. `packages/manager-controller/src/agent/tools.ts` `create_run` handler (lines 1330-1443):
      - Replace the gate `!isValidTransition(currentPhase, 'running')` with `!isValidTransition(currentPhase, 'scheduled')` (table edges: `pending → scheduled`, `rework-requested → scheduled`).
      - Delete the direct run-creation body: the `buildWorkerRun` call, `resolvePhaseAgent`, agent/model override handling (lines 1342-1351, 1375-1385), `validateModelAuth` (1387-1396), the `createRun` + `AlreadyExists` delete/recreate handling (1414-1434), and the rollback patch — `create_run` must never call `createRun`.
      - Keep the initial read (`getTask`); re-read the task inside the handler (mirroring `effects.ts:64-89`) and, if the phase changed since the first read, return a no-op response instead of patching (no lost-update clobber).
      - Idempotency: if `currentPhase` is already `'scheduled'`/`'initializing'`, return a no-op `{ phase: 'Scheduled' }` response ("run already being scheduled by the reconciler").
      - For any other phase, keep the existing-style error naming allowed transitions and pointing at `force_retry`/`set_task_state`.
      - Patch only `Task.status.phase` → `'scheduled'` via `patchTaskStatus` — do **not** pre-populate worker fields (`decideScheduled` computes `runName`/`status`/`gitBranch`/`parentBranch`/`mergeIntoBranch` deterministically from the task; pre-populating would drift).
      - Return `{ project, task, phase: 'Scheduled', expectedRunName: workerRunName(projectName, taskName, worker.retryCount ?? 0, worker.aiReworkCount ?? 0), note: 'run will be created by the reconciler on its next reconcile cycle' }` — same formula as `decision.ts:271` so callers can poll.
      - Remove the `agent`/`model` input args (and their schema entries, lines 240-247) and the `retryCount`/`reworkFeedback` args — overrides and feedback only make sense at run-build time, which is now the reconciler's job (`force_retry` keeps the override capability for the restart path). Update `validateAgentTaskCapability` accordingly (no override → no capability check needed in this handler).
   2. Update the tool description (tools.ts:232-234) truthfully, e.g.: "Schedule a task now (admin shortcut). Moves a 'pending' (backlog) or 'rework-requested' task to 'scheduled' via the standard transition table; the reconciler creates the Run on its next reconcile cycle. Does not create Run CRs itself — all runs are created by the reconciler. For failed/stuck tasks use `force_retry`; for arbitrary phase moves use `set_task_state`."
   3. Sync the secondary doc sites: AGENTS.md `create_run` section (lines 577-583 — replace "Direct run creation without waiting for reconcile cycle" and the "moves task to running" bullets with the schedule-through-reconciliation contract; drop "use `force_retry` first" for `pending` and add the `pending`/`rework-requested` → `scheduled` semantics); `docs/reference/mcp-tools.md` row 15 ("Create a new run for a ready task" → "Schedule a backlog task now; the reconciler creates the run") and line 244 ("`create_run` (agent override)" → remove from the override list or reword to "`create_run` (schedule backlog)"). `docs/task-lifetime.md:8` and `docs/architecture.md:63` are generic and stay accurate — no change.
   4. Add success-path tests (fixes C7) in a new `packages/manager-controller/src/agent/__tests__/tools-create-run.test.ts` (mock `@percussionist/kube` as in `tools-capability-gating.test.ts`; call via the existing `__test.handleMcp` harness, `tools.ts:2887-2889`):
      - `pending` task → `patchTaskStatus` called with `{ phase: 'scheduled' }` exactly, `createRun` spy **never** called, response `phase: 'Scheduled'` with the expected deterministic run name;
      - `rework-requested` task → same;
      - `pending` task whose re-read shows `scheduled` (concurrent reconciler won the race) → no-op response, no patch;
      - already-`scheduled`/`initializing` task → no-op response, no error, no patch;
      - `running`, `waiting-for-input`, `failed`, `done` → rejected with the allowed-transitions error naming `force_retry`/`set_task_state`, no patch, no run creation;
      - **pipeline pin:** a `reconcile.test.ts`-style test asserting that after the tool patches to `scheduled`, `decide()` returns `scheduled → initializing` + `ScheduleRun` effect and `executeEffects` creates the Run — this pins the "all runs through reconciliation" property end-to-end.
    *Files:* `packages/manager-controller/src/agent/tools.ts`, `AGENTS.md`, `docs/reference/mcp-tools.md`, new `tools-create-run.test.ts`, `reconciler/__tests__` additions.
...
- **A3 decision (rev 3) — `create_run` schedules through reconciliation; no transition holes.** The rev-2 approach (a `pending → running` exception in the handler) was rejected: *"all runs should go through reconciliation; no poking holes to the allowed transitions."* The rev-3 design complies on both counts: the tool only performs existing table transitions (`pending`/`rework-requested` → `scheduled`) and never calls `createRun` — the reconciler's `ScheduleRun` effect is the sole run-creation path, which the new pipeline-pin test asserts. Trade-offs accepted:
  - **WIP gate bypass:** `decideScheduled` has no capacity check (only `decidePending`/`decideReworkRequested` do), so a task flipped straight to `scheduled` gets its run created regardless of `maxParallel`. This is the intended "skip the queue" admin semantic — document it in the tool description.
  - **Overrides removed:** `create_run` no longer accepts `agent`/`model` (they were applied to the Run spec at build time, which the tool no longer does). `force_retry` retains agent/model override for the restart path. If maintainers later want overrides on the schedule path, that needs task-annotation plumbing (`percussionist.dev/agent-override` etc.) honored by `buildWorkerRun` — flagged as a follow-up, deliberately out of scope here to keep BUILD-3 small and safe.
  - **Race:** tool patch vs. concurrent reconciler `pending → scheduled` are now the *same* transition — converging on `scheduled` is idempotent; only one `ScheduleRun` effect fires (the decision runs on the observed phase), and `effects.ts` swallows `AlreadyExists` on the run create. The re-read-before-patch in the handler prevents clobbering a later phase.
  - **`force_retry` note:** `force_retry` (createRun: true, tools.ts:1582) remains a direct-run-creation path, kept as an explicitly-administered exception with an override log. Per review scope it is **not** changed here; flag for a future maintainer decision on whether it should also be converted to schedule-through-reconciliation.
