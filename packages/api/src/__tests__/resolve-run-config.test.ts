...
describe('resolveRunConfig', () => {
  ...
  const baseBoard = {
    ...
    image: 'board-image',
    timeoutSeconds: 900,
    resources: { requests: { cpu: '200m' } } as ResourceRequirements,
  };
  const baseRun = {
    model: 'run-model',
    image: 'run-image',
    timeoutSeconds: 300,
    resources: { requests: { memory: '256Mi' } } as ResourceRequirements,
  };
  const baseCluster = {
    runner: {
      image: 'cluster-image',
      timeoutSeconds: 7200,
      resources: { requests: { cpu: '100m' } } as ResourceRequirements,
    },
    secrets: undefined,
  };
  ...
  it('clusterBase is the lowest priority fallback', () => {
    const minimalProject: ProjectSpec = {
      source: { git: { url: 'https://github.com/test/repo.git' } },
    };
  ...
  it('partial board overrides fill only those fields', () => {
    ...
    const boardPartial = { timeoutSeconds: 42 } as {
      model?: string;
      image?: string;
  ...
  it('secrets cascade: run > project > cluster', () => {
    const _clusterSecrets = { secrets: { githubToken: 'cluster' } as Record<string, string> };
