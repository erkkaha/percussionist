...
program
  .command('web')
  .description(
    'open the percussionist dashboard in your browser via localhost port-forward (enables browser notifications and audio)',
  )
...
program
  ...
  .option('--auth-key <key>', 'key inside --auth-secret holding auth.json (default: auth.json)')
...
program
  ...
  .option('-n, --namespace <ns>', 'namespace', DEFAULT_NAMESPACE)
  .option('--timeout <seconds>', 'abort with exit code 2 if not settled within N seconds', '600')
  .option(
    '--for <phase>',
...
const sshKey = program
  .command('ssh-key')
  .description('manage SSH key Secrets for private git repos');

sshKey
  .command('create')
  .description('create or update a kubernetes.io/ssh-auth Secret from a local SSH private key')
  .option('-n, --namespace <ns>', 'namespace', DEFAULT_NAMESPACE)
  .option('--name <name>', 'Secret name to create/update', 'git-ssh-key')
  .option(
    '--key <path>',
...
project
  ...
  .option('--llm-keys-secret <name>', 'Secret with provider API keys')
  .option('--auth-secret <name>', 'Secret with opencode auth.json (from `beatctl auth import`)')
...
const agent = program
  .command('agent')
  .description('manage ClusterAgent resources (cluster-wide agent catalog)');
...
const board = program
  .command('board')
  .description('manage the kanban board embedded in an Project');
