...
import type { FilePart, RawMessage, ToolPart, ToolResultPart, ToolUsePart } from './session.js';
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (attempt < MAX_ATTEMPTS) {
          err(
            `sendStats: web pod HTTP ${res.status} (attempt ${attempt}/${MAX_ATTEMPTS}), retrying: ${body}`,
          );
        ...
        log(
          `sendStats: persisted ${sessionID} — ${messagesPayload.length} messages, ${toolCallsPayload.length} tool calls`,
        );
      }
      return;
    } catch (e) {
      if (attempt < MAX_ATTEMPTS) {
        err(
          `sendStats: POST failed (attempt ${attempt}/${MAX_ATTEMPTS}), retrying:`,
          (e as Error).message,
        );
        await new Promise((r) => setTimeout(r, 500 * 2 ** (attempt - 1)));
        continue;
      ...
      if (part.type === 'tool') {
        ...
        if (refId) {
          ...
          if (existing) {
            ...
            if (rp.isError) {
              existing.error =
                typeof rp.content === 'string' ? rp.content : JSON.stringify(rp.content);
            }
