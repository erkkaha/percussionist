...
mock.module("../agent/memory-client.js", () => ({
  storeMemory: async (_project: string, _content: string, _metadata?: Record<string, unknown>, _agentRun?: string) => {
...
const mockConfigMaps = new Map<string, { name: string; namespace: string; data?: Record<string, string> }>();
...
function createFakeCoreV1Api() {
  return {
    ...
    patchNamespacedConfigMap: async ({ name, namespace, body }: {
      ...
      if (!cm) {
        cm = { name, namespace, data: {} } as unknown as import("@kubernetes/client-node").V1ConfigMap;
...
// ---------------------------------------------------------------------------
// Mock opencode session functions — avoid real LLM calls.
// ---------------------------------------------------------------------------

mock.module("../agent/session.js", () => ({
  createSession: async (title: string) => `session-${title}`,
  sendPrompt: async (_sessionId: string, _prompt: string) => { /* no-op */ },
  waitForCompletion: async (_sessionId: string, _timeoutMs?: number) => {
...
describe("summarizeSession — memory-write failure", () => {
  ...
  it("succeeds despite storeMemory throwing (non-fatal degradation)", async () => {
    ...
    seedConfigMap(
      ...
      [
        ...
        {
          info: { role: "assistant" },
          parts: [{ type: "text", text: "Done. Added storeMemory, queryMemory, and getContext functions." }],
        },
      ],
    );
  ...
  it("logs a warning containing project/run/session/error when storeMemory fails", async () => {
    ...
    const memoryWarning = errorLogs.find((log) =>
