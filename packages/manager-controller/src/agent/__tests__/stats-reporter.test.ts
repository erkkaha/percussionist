      ...
      if (urlString.includes("/session/session-abc/message") && !urlString.includes("stats")) {
        ...
        return new Response(
          JSON.stringify([
            ...
            { info: { role: "assistant", tokens: { output: 20 }, time: { completed: Date.now() } }, parts: [{ type: "text", text: "Hi" }] },
          ]),
    ...
    globalThis.fetch = async (_url: any, _init?: any) => {
      ...
      return new Response(
        JSON.stringify([
          ...
          { info: { role: "assistant", tokens: { output: 5 } }, parts: [{ type: "text", text: "X" }] },
        ]),
        { status: 200, headers: { "Content-Type": "application/json" } },
      );
    };
  ...
  it("should send correct payload structure with agent = 'manager run'", async () => {
    ...
    globalThis.fetch = async (url: any, init?: any) => {
      ...
      if (urlString.includes("/session/session-abc/message") && !urlString.includes("stats")) {
        return new Response(
          JSON.stringify([
            { info: { role: "assistant", tokens: { input: 5, output: 10 }, cost: 0.02 }, parts: [{ type: "text", text: "Hello" }] },
          ]),
          { status: 200, headers: { "Content-Type": "application/json" } },
        );
      }
  ...
  it("should handle HTTP error from web pod non-fatally", async () => {
    globalThis.fetch = async (url: any, _init?: any) => {
      ...
      if (urlString.includes("/session/session-abc/message") && !urlString.includes("stats")) {
        return new Response(
          JSON.stringify([
            { info: { role: "assistant", tokens: { output: 5 } }, parts: [] },
          ]),
          { status: 200, headers: { "Content-Type": "application/json" } },
        );
      }
  ...
  it("should issue POST with phase Succeeded on success", async () => {
    ...
    globalThis.fetch = async (url: any, init?: any) => {
      ...
      if (urlString.includes("/session/session-abc/message") && !urlString.includes("stats")) {
        return new Response(
          JSON.stringify([
            { info: { role: "assistant", tokens: { input: 10, output: 20 }, cost: 0.05 }, parts: [{ type: "text", text: "Done" }] },
          ]),
          { status: 200, headers: { "Content-Type": "application/json" } },
        );
      }
    ...
    globalThis.fetch = async (url: any, init?: any) => {
      ...
      if (urlString.includes("/session/session-abc/message") && !urlString.includes("stats")) {
        ...
        return new Response(
          JSON.stringify([
            { info: { role: "user", tokens: { input: 5 } }, parts: [] },
            { info: { role: "assistant", tokens: { output: 10 }, time: { completed: Date.now() } }, parts: [{ type: "text", text: "Done" }] },
          ]),
          { status: 200, headers: { "Content-Type": "application/json" } },
        );
      }
  ...
  it("should call final flush with Succeeded phase on completion", async () => {
    ...
    globalThis.fetch = async (url: any, init?: any) => {
      ...
      if (urlString.includes("/session/session-abc/message") && !urlString.includes("stats")) {
        ...
        return new Response(
          JSON.stringify([
            { info: { role: "assistant", tokens: { output: 10 }, time: { completed: Date.now() } }, parts: [{ type: "text", text: "Ready" }] },
          ]),
          { status: 200, headers: { "Content-Type": "application/json" } },
        );
      }
