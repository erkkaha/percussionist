...
export async function buildBuildTaskGeneratorRun(
  ...
  const promptLines = [
    ...
    `create_task tool for each BUILD task that implements the plan. Work ONLY from`,
    ...
    ...(availableAgents.length > 0
      ? [
          ...
          `For each BUILD task, specify the agent via the create_task "agent" parameter.`,
    ...
    "",
    `If the context above is insufficient to derive concrete BUILD tasks, call complete_run`,
    ...
    `2. Call create_task for each task IN ORDER:`,
    ...
    `   - predecessorRef: the taskName returned by a previous create_task call if this task depends on it`,
    `3. Each create_task returns { taskName, ... }. Use the returned taskName as predecessorRef for dependent tasks.`,
    ...
    `- If the PLAN requires no BUILD tasks (was purely research/planning), call complete_run with summary "no build tasks required"`,
    "",
    `CRITICAL — DO NOT:`,
