...
export function prUpdatePromptLines(
  ...
  return [
    ...
    '',
    '## Reply in the review threads',
    '',
    'Reviewers work thread-by-thread, so a summary comment alone leaves their',
    'findings looking ignored. Find the inline review threads that have no reply',
    'from your own account:',
    `    gh api user --jq .login`,
    `    gh api "repos/{owner}/{repo}/pulls/${prNumber}/comments?per_page=100" \\`,
    "      --jq '.[] | {id, in_reply_to_id, user: .user.login, path, body}'",
    '(a thread is one root comment — `in_reply_to_id` null — plus its replies).',
    '',
    'For each unanswered thread that the commits you just pushed address, reply',
    'IN THAT THREAD with 1-3 sentences: the commit sha and what actually changed,',
    'grounded in the diff — never a bare "fixed". Post a reply with:',
    `    gh api --method POST "repos/{owner}/{repo}/pulls/${prNumber}/comments" \\`,
    "      -f body='Fixed in <sha>: <what changed>.' -F in_reply_to=<root-comment-id>",
    '',
    'Leave threads the rework did not address without a reply — never claim a fix',
    'you cannot point to in the diff.',
    '',
    '## Comment on the PR',
    '',
    'Then post the roll-up for the human reviewers with:',
