...
export async function warmupModel(): Promise<void> {
  ...
  for (let attempt = 0; attempt <= cfg.maxRetries; attempt++) {
    if (attempt > 0) {
      ...
      const baseDelayMs = Math.min(1_000 * 2 ** (attempt - 1), 30_000);
      const jitter = Math.random() * baseDelayMs * 0.5;
      const delay = baseDelayMs + jitter;
      console.log(`[memory] warmup: retry ${attempt}/${cfg.maxRetries} in ${Math.round(delay)}ms`);
      await sleep(delay);
    ...
    try {
      ...
      if (modelExists(tags, cfg.model)) {
        console.log(`[memory] warmup: model "${cfg.model}" already present (${tags.length} total)`);
        _ready = true;
        return;
      }
