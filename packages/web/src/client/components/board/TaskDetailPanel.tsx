...
const planMarkdownComponents: React.ComponentProps<typeof ReactMarkdown>['components'] = {
  ...
  code: ({ children, className }) => {
    ...
    return (
      <span className="bg-surface-sunken rounded px-1 py-0.5 text-xs font-mono">{children}</span>
    );
  },
...
const taskDescriptionMarkdownComponents: React.ComponentProps<typeof ReactMarkdown>['components'] =
  {
    ...
    h1: ({ children }) => (
      <h1 className="text-headline-md font-semibold mt-3 mb-1 text-text">{children}</h1>
    ),
    h2: ({ children }) => (
      <h2 className="text-body-lg font-semibold mt-2 mb-1 text-text">{children}</h2>
    ),
    h3: ({ children }) => (
      <h3 className="text-body-sm font-semibold mt-2 mb-0.5 text-text">{children}</h3>
    ),
    ...
    a: ({ children, href }) => (
      <a
        ...
        className="text-accent hover:text-accent/80 underline decoration-dotted"
      >
        {children}
      </a>
    ),
...
function PlanContent({ projectName, taskName }: { projectName: string; taskName: string }) {
  ...
  return (
    <div className="text-xs max-w-none px-4 py-3" style={{ fontSize: '12px', lineHeight: '1.5' }}>
      <ReactMarkdown remarkPlugins={remarkPlugins} components={planMarkdownComponents}>
        {data.content}
      </ReactMarkdown>
    </div>
  );
    ...
    <div className="space-y-1">
      {commits.map((commit) => {
        ...
        return (
          <div
            key={commit.sha}
            className="rounded border border-border-muted bg-surface overflow-hidden"
          >
            <button
...
function OverviewContent({
  ...
  const allRuns = [...(runsData ?? [])].sort((a, b) => {
    const aTime =
      a.status?.completedAt ?? a.status?.startedAt ?? a.metadata.creationTimestamp ?? '';
    const bTime =
      b.status?.completedAt ?? b.status?.startedAt ?? b.metadata.creationTimestamp ?? '';
      ...
      {task.spec.description && (
        <div>
          ...
          <ReactMarkdown
            ...
            components={taskDescriptionMarkdownComponents}
          >
            {task.spec.description}
          </ReactMarkdown>
        </div>
              ...
              {task.status.reviews.map((r) => {
                ...
                const actionColor =
                  r.action === 'approve'
                    ...
                    : r.action === 'request_changes'
                      ? 'text-phase-pending border-phase-pending/30 bg-phase-pending/10'
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        <span
                          className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${actionColor}`}
                        >
                          {r.action === 'approve' ? (
          ...
          <div className="space-y-1">
            {allRuns.map((run) => {
              ...
              const duration = startedAt
                ...
                : null;
              const hasTokens =
                run.status?.tokensIn !== undefined || run.status?.tokensOut !== undefined;
                    ...
                    <div className="border-t border-border-muted px-3 py-2 space-y-2">
                      ...
                      {(hasTokens || duration) && (
                        <div className="flex items-center gap-3 text-xs text-text-dim">
                          {hasTokens && (
                            <span>
                              Tokens: {run.status?.tokensIn ?? 0} in / {run.status?.tokensOut ?? 0}{' '}
                              out
                            </span>
                          )}
      ...
      <div className="shrink-0 px-4 pt-4 pb-3 border-b border-border space-y-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {task.spec.priority && task.spec.priority !== 'medium' && (
            <span
              className={`text-label-md font-mono uppercase px-1.5 py-0.5 rounded flex items-center gap-0.5 ${
                task.spec.priority === 'high'
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center p-8 border-l border-border">
      ...
      <p className="text-xs text-text-dim/60 mt-1">
        Session, logs, and plan artifacts will appear here
      </p>
    </div>
  );
}
