    ...
    <div className="space-y-5">
      ...
      <fieldset className="space-y-3 rounded-md border border-border p-4">
        ...
        <p className="text-xs text-text-dim">
          ...
          The agent reaches them via <code className="font-mono">localhost</code>. opencode waits
          for all declared ports to be reachable before starting.
        </p>
        ...
        {form.sidecars.length > 0 && (
          <div className="space-y-4">
            {form.sidecars.map((sc, idx) => (
              <div
                key={sc.id}
                className="rounded-md border border-border-muted p-3 space-y-3 relative"
              >
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-text-muted">
                    Ports <span className="text-text-dim font-normal">(comma-separated)</span>
                  </label>
                  <Input
                    type="text"
                    value={sc.ports}
      ...
      <fieldset className="space-y-3 rounded-md border border-border p-4">
        ...
        <p className="text-xs text-text-dim">
          ...
          Content is stored as K8s Secrets. Useful for <code className="font-mono">.env</code> files
          or other config files the agent needs.
        </p>
      ...
      <fieldset className="space-y-3 rounded-md border border-border p-4">
        ...
        <p className="text-xs text-text-dim">
          Shell script to run after git clone completes, before opencode starts. Runs in the init
          container — failure (non-zero exit) will prevent the pod from starting. Working directory
          is <code className="font-mono">/workspace</code> (the cloned repo root).
        </p>
        <Textarea
