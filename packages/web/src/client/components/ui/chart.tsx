...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  return (
    <style
      // biome-ignore lint/security/noDangerouslySetInnerHtml: Chart CSS is generated from typed color config, not user HTML.
        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            const indicatorColor =
              color ??
              ((i.payload as Record<string, unknown>)?.fill as string) ??
              (i.color as string);

            return (
              <div
                key={i.dataKey as string}
                className={cn(
                    ...
                    >
                      <div className="grid gap-1.5">
                        ...
                        <span className="text-muted-foreground">
                          {itemConfig?.label ?? (i.name as string)}
                        </span>
                      </div>
                      {(i.value as number) ? (
                        <span className="font-mono font-medium tabular-nums text-foreground">
                          {(i.value as number).toLocaleString()}
                        </span>
                      ) : null}
                    </div>
                  </>
          ...
          >
            {itemConfig?.icon && !hideIcon ? (
              ...
              <div
                ...
                style={{
                  backgroundColor: i.color as string,
                }}
              />
            )}
            {itemConfig?.label}
