...
const MCP_URL = `http://percussionist-manager.${NAMESPACE}.svc.cluster.local:4097/mcp`;
  ...
  const mcpRequest = {
    ...
    params: {
      ...
      arguments: {
        ...
        namespace: NAMESPACE,
        // skipSanitization: true — the diff script is generated by our backend
        // (not from user/agent input) and legitimately uses $(), pipes, &&, and
        // other shell constructs that the security sanitizer would block.
        // The manager honors this only because managerMcpHeaders() presents the
