...
describe("unauthenticated access → 401", () => {
  ...
  describe("stats", () => {
    ...
    it("POST /api/stats/tool-events → 401", async () =>
      expectUnauthorized("/api/stats/tool-events", "POST", { events: [] }));
    ...
    it("GET /api/stats/tool-events → 401", async () =>
      expectUnauthorized("/api/stats/tool-events"));
  });
