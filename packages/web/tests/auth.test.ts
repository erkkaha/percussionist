...
function _json(path: string, body: unknown, method = 'POST') {
  return req(path, {
    method,
...
async function expectUnauthorized(path: string, method = 'GET', body?: unknown): Promise<void> {
...
async function _expectAuthenticated(
  path: string,
  method = 'GET',
  body?: unknown,
): Promise<Response> {
...
async function expectWrongTokenRejected(path: string, method = 'GET'): Promise<void> {
  const res = await req(path, {
    method,
...
describe('unauthenticated access → 401', () => {
  ...
  describe('settings (read)', () => {
    ...
    it('GET /api/settings/secrets → 401', async () => expectUnauthorized('/api/settings/secrets'));
  });
  ...
  describe('projects', () => {
    ...
    it('GET /api/projects/events → 401', async () => expectUnauthorized('/api/projects/events'));
    ...
    it('GET /api/projects/myproj → 401', async () => expectUnauthorized('/api/projects/myproj'));
  ...
  describe('runs', () => {
    ...
    it('GET /api/runs/events → 401', async () => expectUnauthorized('/api/runs/events'));
    it('GET /api/runs/myrun → 401', async () => expectUnauthorized('/api/runs/myrun'));
    it('POST /api/runs → 401', async () => expectUnauthorized('/api/runs', 'POST', { task: 't1' }));
    it('DELETE /api/runs/myrun → 401', async () => expectUnauthorized('/api/runs/myrun', 'DELETE'));
  ...
  describe('agents', () => {
    ...
    it('GET /api/agents/events → 401', async () => expectUnauthorized('/api/agents/events'));
    it('GET /api/agents/myagent → 401', async () => expectUnauthorized('/api/agents/myagent'));
  ...
  describe('upgrade', () => {
    it('GET /api/upgrade/status → 401', async () => expectUnauthorized('/api/upgrade/status'));
  ...
  describe('agent-chat', () => {
    ...
    it('GET /api/agent/status → 401', async () => expectUnauthorized('/api/agent/status'));
  });
  ...
  describe('logs', () => {
    it('GET /api/runs/myrun/logs → 401', async () => expectUnauthorized('/api/runs/myrun/logs'));
  });
  ...
  describe('metrics', () => {
    it('GET /api/metrics/nodes → 401', async () => expectUnauthorized('/api/metrics/nodes'));
    it('GET /api/metrics/pods → 401', async () => expectUnauthorized('/api/metrics/pods'));
    it('GET /api/metrics/events → 401', async () => expectUnauthorized('/api/metrics/events'));
  });

  // Providers route — need auth (LLM provider config)
  describe('providers', () => {
    it('GET /api/providers → 401', async () => expectUnauthorized('/api/providers'));
  });

  // Activity route — needs auth (task event history)
  describe('activity', () => {
    it('GET /api/activity → 401', async () => expectUnauthorized('/api/activity'));
  });
  ...
  describe('board (mutating)', () => {
    ...
    it('POST /api/projects/myproj/board/tasks → 401', async () =>
      expectUnauthorized('/api/projects/myproj/board/tasks', 'POST', {
        type: 'PLAN',
        title: 't',
        agent: 'planner',
      }));
    ...
    it('POST /api/projects/myproj/board/task-events → 401', async () =>
      expectUnauthorized('/api/projects/myproj/board/task-events', 'POST', {
        taskName: 't1',
        taskType: 'PLAN',
        eventType: 'created',
      }));
