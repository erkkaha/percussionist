...
async function pollPhase(runName: string, ns: string, expected: string): Promise<string | null> {
...
async function pollTaskPhase(taskName: string, ns: string, expected: string): Promise<true | null> {
...
describe('basic-failure', () => {
  beforeAll(async () => {
    ...
    await applyClusterAgents(['clusteragent-failing-worker.yaml']);
  ...
  source:
    ...
    await applyTask({
      ...
      description:
        'This task is intentionally designed to fail so that the failure path is exercised.',
    });
  });
  ...
  it('worker run reaches Failed', async () => {
    // git-clone init container fails on invalid URL; allow up to 300s.
    await waitFor(`worker run ${workerRun} reaches Failed`, 300, 10, () =>
  ...
  it('task phase reaches failed', async () => {
    // Manager reconciles on a ~30s periodic resync; allow up to 180s.
    await waitFor(`task ${TASK_NAME} phase=failed`, 180, 10, () =>
