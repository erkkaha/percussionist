...
describe("review-findings", () => {
  ...
  it(
    "reviewer findings are persisted in Task.status.diffFindings",
    async () => {
      await waitFor(
        `task ${TASK_NAME} has diffFindings`,
        180,
        3,
  ...
  it(
    "web diff API returns findings with active/stale mapping",
    async () => {
      ...
      const findings = (payload as { findings: Array<{ id: string; isActive: boolean; isStale: boolean }> }).findings;
